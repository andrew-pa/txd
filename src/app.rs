...
impl TxdApp {
    pub fn init(mut rx: &mut RenderContext) -> TxdApp {
        ...
        let buf = Rc::new(RefCell::new(Buffer::load(Path::new("src\\main.rs"), rx, res.clone()).expect("open file")));
        TxdApp { state: State { buf }, res, mode: Box::new(mode::NormalMode::new()) }
    }
}
...
impl App for TxdApp {
    ...
    fn paint(&mut self, mut rx: &mut RenderContext) {
        ...
        let bnd = rx.bounds();
        let mut buf = self.state.buf.borrow_mut();
